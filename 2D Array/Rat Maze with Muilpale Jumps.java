...
Output: [[-1]]
Explanation: The rat starts at (0, 0) with value 2, but every possible path from there eventually reaches a cell containing 0. Since no sequence of jumps
can reach the destination cell (3, 3), no valid path exists and the output is [[-1]].
...
class Solution {
    ...
    private boolean isSafe(int r, int c, int[][] maze, int n) {
        return (r >= 0 && r < n && c >= 0 && c < n && maze[r][c] != 0);
    }

    private boolean solveRec(int r, int c, int[][] maze, int n) {
        if (r == n - 1 && c == n - 1) {
            arr[r][c] = 1;
            return true;
        }
