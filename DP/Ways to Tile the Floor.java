...
Input: n = 2, m = 3
Output: 1
Explanation: Placing a tile vertically would require a height of 3, which exceeds the floor's height of 2. The only way to cover the entire floor is by
placing 2 horizontally, one in each row, resulting in exactly one valid tiling. 
...
class Solution {
   private static final int MOD = 1000000007;
    ...
    int countWays(int n, int m) {
        ...
        for (int i = 0; i < m; i++) dp.addLast(1);
    
        for (int i = m; i <= n; i++) {
            int ways;
            if (i == m)  ways = 2;
            else ways = (dp.getLast() + dp.getFirst()) % MOD;
